...
| R2 | `application.yml` 配置 Redis 连接 | [ ] 待完成 |
| R3 | 使用 `StringRedisTemplate` 实现 `SET key NX EX` 封装 | [ ] 待完成 |
| R4 | `MessageController` 改为调用 Redis 幂等（保留内存实现可注释或删除） | [ ] 待完成 |
| R5 | 本地验证：同 `X-Idempotency-Key` 仅首次入队 | [ ] 待完成 |

说明：当前工程已实现**内存版**幂等（`ConcurrentHashMap`），作为对照；Redis 版上线后应关闭内存版，避免双轨逻辑。
...
预期现象：
- 首次某幂等键：消息入队并被消费；重复同键：接口返回重复忽略，不再重复入队（内存版或 Redis 版以当前实现为准）
...
1. 完成 Phase 3 中 **Redis 幂等**（R1～R5），替换内存版幂等实现。
2. 进阶补充 **MySQL 幂等表** 与审计字段，对比 Redis 方案的取舍。
3. 深化 **DLQ 运维**：检索、回放、告警与人工补偿流程。
